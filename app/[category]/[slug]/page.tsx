...
// Komponen renderer untuk isi artikel (Jeda paragraf & Gambar)
const ptComponents = {
  types: {
    image: ({ value }: any) => {
  ...
  block: {
    ...
    h2: ({ children }: any) => (
      <h2 style={{ marginTop: '2.5rem', marginBottom: '1.2rem', color: '#004a8e', fontSize: '24px' }}>
        {children}
      </h2>
    ),
  },
...
export default async function PostDetail({ 
  ...
  const shareUrl = `https://abahsaif.com/${category}/${slug}`;
  ...
  return (
    <article style={{ maxWidth: '800px', margin: '40px auto', padding: '0 20px' }}>
      ...
      <header style={{ marginBottom: '30px' }}>
        ...
        {/* BARIS PENULIS & SHARE MEDSOS RATA KANAN */}
        <div style={{ 
          display: 'flex', 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <span style={{ fontSize: '12px', color: '#888' }}>
                {new Date(post.publishedAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })} pukul 07.55 WIB
              </span>
            </div>
          </div>

          {/* SISI KANAN: TOMBOL SHARE DENGAN IKON PUTIH */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', border: '1px solid #ddd', padding: '6px 12px', borderRadius: '4px', fontSize: '13px', fontWeight: 'bold', color: '#444', backgroundColor: '#fff' }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              {/* Facebook - Ikon Putih */}
              <a href={`https://www.facebook.com/sharer/sharer.php?u=${shareUrl}`} target="_blank" style={{ backgroundColor: '#1877F2', width: '36px', height: '36px', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
              ...
              {/* X (Twitter) - Ikon Putih */}
              <a href={`https://twitter.com/intent/tweet?url=${shareUrl}`} target="_blank" style={{ backgroundColor: '#000', width: '36px', height: '36px', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
              ...
              {/* WhatsApp - Ikon Putih */}
              <a href={`https://api.whatsapp.com/send?text=${shareUrl}`} target="_blank" style={{ backgroundColor: '#25D366', width: '36px', height: '36px', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
      ...
      {/* 3. RELATED POSTS (PAS 3 POSTINGAN) */}
